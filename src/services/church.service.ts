...
export const statusServiceDb = async (tenantId: string, serviceId: string) => {
    try {
...
export const getLivestreamStatusDb = async (tenantId: string, serviceId: string) => {
    try {
        const sessionRepository = AppDataSource.getRepository(Session);
        const sessionId = await SessionService.getActiveSession(tenantId, serviceId);
        if (!sessionId) throw new Error (`Active Session ID not found`);

        const session = await sessionRepository.findOne({where: { id: sessionId}});
        if (!session) throw new Error(`Session not found with this ID`);
