...
export class SessionService {
    ...
    static async getActiveSession(tenantId, serviceId) {
        try {
            const sessionRepository = AppDataSource.getRepository(Session);
            const session = await sessionRepository
    ...
    static async stopOldSessions(tenantId, serviceId) {
        try {
            const sessionRepository = AppDataSource.getRepository(Session);
    ...
    static async addLanguageToSession(sessionId, language) {
        try {
            const sessionRepository = AppDataSource.getRepository(Session);
    ...
    static async removeLanguageFromSession(sessionId, language) {
        try {
            const sessionRepository = AppDataSource.getRepository(Session);
            const session = await sessionRepository.findOne({ where: { id: sessionId } });
            if (!session)
